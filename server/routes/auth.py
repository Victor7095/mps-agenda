...
def sign_up():
  try:
    ...
    user_data = request_data["user"]
    user = user_schema.load(user_data)  # type: User
...
def send_password_reset_email():
  try:
    client_address = f"{os.environ['CLIENT_ADDRESS']}/{os.environ['CLIENT_PASSWORD_RESET_PATH']}"
    request_data = request.get_json()
    ...
    token = create_access_token(
        identity=identity, expires_delta=expires_in, additional_claims={"type": "password"})

    link = f'{client_address}?token={token}'

    msg = Message("Rango Legal - Reset Password",
                  recipients=[email])
